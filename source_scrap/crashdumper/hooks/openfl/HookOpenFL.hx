...
class HookOpenFL implements IHookPlatform
{
	...
	public function new()
	{
		...
		version = Util.getProjectVersion("Project.xml"); // JOELwindows7: filename case sensitivity issues!!! `P` != `p`.
		#end
		#end
		#else
	...
	public function getFolderPath(str:String):String
	{
		...
		{
			case null, "":
				str = SystemPath.applicationStorageDirectory;
			case PATH_APPDATA:
				str = SystemPath.applicationStorageDirectory;
			case PATH_DOCUMENTS:
				str = SystemPath.documentsDirectory;
			case PATH_DESKTOP:
				str = SystemPath.desktopDirectory;
			case PATH_USERPROFILE:
				str = SystemPath.userDirectory;
			case PATH_APP:
				str = SystemPath.applicationDirectory;
		}
		#end
		if (str != "")
		{
