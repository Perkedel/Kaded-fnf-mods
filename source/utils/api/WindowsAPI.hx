...
// JOELwindows7: yey
// https://github.com/FNF-CNE-Devs/YoshiCrafterEngine/blob/main/source/WindowsAPI.hx#L80
// https://community.openfl.org/t/transparent-desktop-window-background-confusion/11026/8
// thancc Yoshi, if you don't mind if I borrow this.., iYEOINK!
#if (windows && cpp)
@:buildXml('
    ...
    switch(Message)
    {
        ...
        case WM_INITDIALOG:
            ...
            reportIssue = CreateWindow("static",
                "We recommend reporting the issue to the GitHub page:", // A copy of the error has been saved in \"crash.txt\" ${Perkedel.ENGINE_BUGREPORT_URL}
            ...
            githubLink = CreateWindow("static",
                "${Perkedel.ENGINE_BUGREPORT_URL}", // A copy of the error has been saved in \"crash.txt\"
        ...
        case WM_COMMAND:
            if ((HWND)lParam == githubLink) {
                ShellExecute(NULL, "open", "${Perkedel.ENGINE_BUGREPORT_URL}", NULL, NULL, SW_SHOWNORMAL);
                return TRUE;
...
class WindowsAPI
{
	...
	// #if (windows && (cpp || java || cs))
	#if (windows && cpp)
	@:functionCode('
    HKEY hKey;
    LPCTSTR data;

    RegCreateKeyEx(HKEY_CURRENT_USER, "SOFTWARE\\\\Classes\\\\${Perkedel.ENGINE_ID_CLOSE}", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, NULL);

    char const *name = "Last Funkin Moments";
    RegSetValueEx(hKey, "", 0, REG_SZ, (BYTE*)name, strlen(name));
    RegSetValueEx(hKey, "FriendlyTypeName", 0, REG_SZ, (BYTE*)name, strlen(name));


    RegCreateKeyEx(HKEY_CURRENT_USER, "SOFTWARE\\\\Classes\\\\${Perkedel.ENGINE_ID_CLOSE}\\\\shell\\\\open\\\\command", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, NULL);
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, "SOFTWARE\\\\Classes\\\\.ycemod", 0, KEY_READ, &ycemodKey) != ERROR_SUCCESS) {
        ...
        char const *name = "LastFunkinMoments";
