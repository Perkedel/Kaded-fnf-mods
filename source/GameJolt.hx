...
class GameJoltAPI // Connects to tentools.api.FlxGameJolt
{
		...
		if (userLogin)
		{
			GJApi.addTrophy(trophyID, function(data:Map<String, String>)
			{
				...
				if (data.exists("message"))
					bool = true;
				if (!bool) // JOELwindows7: peck it. just shut up if you already have one.
					Main.gjToastManager.createToast(GameJoltInfo.imagePath, "Unlocked a new trophy"
						+ (bool ? "... again?" : "!"),
						"Thank you for testing this out!\nCheck out "
						+ advertiseWords[FlxG.random.int(0, advertiseWords.length - 1)]
						+ ", it's cool",
						!bool); // JOELwindows7: sound only on new! also inclusive advertisements
			});
		}
	}

	/**
	 * Nope. IT expert does not agree with with exclusive advertisement.
	 * be **inclusive** yess!
	 * @author JOELwindows7
	 */
	...
	public static function pullHighScore(tableID:Int):Map<String, String>
	{
		...
		GJApi.fetchScore(tableID, function(data:Map<String, String>)
		{
			trace(data);
			returnable = data;
		});
...
// class GameJoltInfo extends CoreState // JOELwindows7: yeah
{
	...
	public static var textArray:Array<String> = [
		...
		// JOELwindows7: now for our part
		"cval_brown I'm sorry! pls unblock me",
		"Kromer",
		"Dafteregf",
		"dafuq",
		"Kris, get the banana",
		"GitHub CoPilot Sentience",
		"Right click, Save as",
		"surthur",
	];
}
